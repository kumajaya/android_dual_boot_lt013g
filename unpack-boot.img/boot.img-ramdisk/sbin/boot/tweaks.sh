...
# simplified and adapted to Galaxy Tab 3 8.0 by Ketut P. Kumajaya
...
if [ -e /proc/sys/net/ipv6/icmp_echo_ignore_broadcasts ]; then # not exists on Galaxy Tab 3 8.0
...
busybox sysctl -w net.ipv4.icmp_ignore_bogus_error_responses=1; # default: 0
if [ -e /proc/sys/net/ipv6/icmp_ignore_bogus_error_responses ]; then # not exists on Galaxy Tab 3 8.0
...
# entropy tweaks
busybox sysctl -w kernel.random.read_wakeup_threshold=512; # default: 64
busybox sysctl -w kernel.random.write_wakeup_threshold=1024; # default: 128
